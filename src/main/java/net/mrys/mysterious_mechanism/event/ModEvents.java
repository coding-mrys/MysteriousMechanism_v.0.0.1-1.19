package net.mrys.mysterious_mechanism.event;
...
@Mod.EventBusSubscriber(modid = mysterious_mechanism.MOD_ID)
public class ModEvents {
    @SubscribeEvent
    public static void addCustomTrades(VillagerTradesEvent event) {
        if(event.getType() == VillagerProfession.FARMER) {
            Int2ObjectMap<List<VillagerTrades.ItemListing>> trades = event.getTrades();
            ItemStack stack = new ItemStack(ModItems.MRYS_FRUIT.get(), 1);
            int villagerLevel = 1;
        ...
        if(event.getType() == ModVillagers.SCIENTIST.get()) {
            Int2ObjectMap<List<VillagerTrades.ItemListing>> trades = event.getTrades();
            ItemStack stack = new ItemStack(ModItems.MRYS_MASK.get(), 1);
            int villagerLevel = 1;
